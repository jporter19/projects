...
if 'col_count' not in st.session_state:
    st.session_state.col_count = None
...
with col1:
    if st.button("DB Reset"):
        ...
        col2.write(resp)
st.divider()
st.subheader(f'Active Collection: [{st.session_state.col_choice}], Document Count: {st.session_state.col_count}')  # Populate section title
...
#st.session_state.col_count = st.session_state.working_col.count()
if st.session_state.col_choice:
    st.session_state.disable = False
else:
    st.session_state.disable = True
if st.session_state.disable == False:
    with st.expander("Collection Information"):
        st.write("Document Count:" + str(st.session_state.working_col.count()))
        st.write(st.session_state.working_col.peek())
