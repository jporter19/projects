...
def delete_collection(col):
    try:
        st.session_state.db_client.delete_collection(col)
...
st.selectbox("Pick a Collection",db_collections(),index=None,key='colpick', on_change=rerun)
st.divider()
col1, col2 = st.columns([1,3])
with col1:
    if st.button("Delete Collection"):
        with col2:
            st.selectbox("Pick a Collection",db_collections(),key='delpick',index=None,)
            if st.session_state.delpick:
                resp = delete_collection(st.session_state.delpick)
                st.write(resp)
